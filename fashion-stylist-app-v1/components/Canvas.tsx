...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys }) => {
  ...
  const handleDownload = () => {
    ...
    link.download = `style-ai-${Date.now()}.png`;
    ...
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      {displayImageUrl && (
        <button 
            ...
            className="absolute top-4 right-4 z-30 flex items-center justify-center text-center bg-gray-900/90 text-white font-semibold p-2.5 rounded-full transition-all duration-200 ease-in-out hover:bg-gray-800 active:scale-95 text-sm shadow-lg"
