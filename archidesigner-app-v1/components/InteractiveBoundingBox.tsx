...
export const InteractiveBoundingBox: React.FC<InteractiveBoundingBoxProps> = ({ box, onBoxChange, bounds, scale = 1 }) => {
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            // Adjust delta by scale to keep movement 1:1 with cursor
    ...
    }, [activeDrag, onBoxChange, bounds, scale]);
    ...
    const getHandleStyle = (name: string): React.CSSProperties => {
        // Handle size needs to be inversely scaled to remain constant visual size? 
        // Or just let them scale with zoom. Scaling with zoom is better for precision usually.
        // Let's keep them scaling for now.
