...
export const AlbumView: React.FC<AlbumViewProps> = ({ album, onUpdateAlbum, onEditImage }) => {
  ...
  const handleSendPrompt = useCallback(
    async (prompt: string, imageFiles: File[], useWebSearch: boolean, aspectRatio: string, sourceImageUrl?: string) => {
      // Allow sending if prompt exists AND (files provided OR we have a cached image to reference)
      if (!prompt || (imageFiles.length === 0 && !sourceImageUrl && !album.lastUploadedImage)) return;
      ...
      let newLastUploadedImage = album.lastUploadedImage;
      
      try {
        if (imageFiles.length > 0) {
            // CASE 1: User uploaded new files.
            imagesData = await readFilesAsBase64(imageFiles);
            // Use Data URLs for display to ensure persistence and avoid revocation issues
            displayImageUrls = imagesData.map(img => `data:${img.mimeType};base64,${img.base64Data}`);
            
            // Update the cache with the first new image
            if (imagesData.length > 0) {
                newLastUploadedImage = imagesData[0];
            }
        } else if (sourceImageUrl) {
            // CASE 2: Explicit source passed (e.g. from Remix).
            const file = await dataUrlToFile(sourceImageUrl, 'source.png');
            imagesData = await readFilesAsBase64([file]);
            displayImageUrls = [sourceImageUrl];
            
            if (imagesData.length > 0) {
                newLastUploadedImage = imagesData[0];
            }
        } else if (album.lastUploadedImage) {
            // CASE 3: Use cached image.
            imagesData = [album.lastUploadedImage];
            // Construct Data URL directly
            displayImageUrls = [`data:${album.lastUploadedImage.mimeType};base64,${album.lastUploadedImage.base64Data}`];
        }

        const primaryImageUrl = displayImageUrls[0];
        ...
        // Update album state immediately with user message, placeholders, and new cached image
        let currentAlbumState = { 
            ...album, 
            chatHistory: updatedChatWithUser, 
            galleryImages: [...album.galleryImages, ...placeholderVariations],
            lastUploadedImage: newLastUploadedImage 
        };
  ...
  const handleRetryVariation = async (failedVariation: ImageVariation) => {
    ...
    try {
        ...
        const newImageBase64 = await retryImageGeneration(images, prompt, aspectRatio);
      ...
      >
        ...
        <div className="p-4 border-t border-black/20 bg-[#131314]">
            <ChatInput
              ...
              hasActiveImage={!!album.lastUploadedImage}
            />
        </div>
      </div>
